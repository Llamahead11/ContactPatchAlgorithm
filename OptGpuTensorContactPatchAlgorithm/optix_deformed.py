...
def inner_deformed_to_inner_undeformed(shared_stream,raycaster,inner_undef_lat,inner_undef,inner_lat,inner_arr,mesh,model_pcd_cuda,t2cam_pcd_cuda,count,model_pcd,model_ply,draw_reg):
    """
    ...
    valid_mask_cp = ((tri_id_t != 0) & (hit_point_t != 0.0).all(axis = 1)).get() 
    ...
    masked_pcd = t2_d_pcd_cu_t.select_by_mask(valid_mask).voxel_down_sample(voxel_size = 0.002) 
    ...
    # filtered_pcd.paint_uniform_color(o3d.core.Tensor([0,0,0]))
    ...
    # plt.xlabel("Distance Deformed (m)")
    # plt.ylabel("Number of points")
    # plt.xlim([-0.020,0.010])
    # plt.ylim([0,1400])
    # plt.title("Histogram of distance deformation")
    # plt.grid(True)
    # plt.tight_layout()
    # #plt.show(block=False)
    ...
    # cp.cuda.Device().synchronize()
    # shared_stream.synchronize()
    return t2_und_t, np.max(d_dist), downsampled_cropped_model_cu, d_dist, masked_pcd#t2_d_pcd_cu_t #1#t2_d_pcd_def
...
def main():
    ...
    try:
        while True:
            ...
            t2_d_pcd_inner_cu, max_inner_def, cropped_model_pcd_cu, d_inner_dist,t2_d_pcd_outer_cu = inner_deformed_to_inner_undeformed(shared_stream,raycaster,inner_undef_lat,inner_undef,inner_lat,inner_arr,t2cam_ply,model_pcd_cuda,t2cam_pcd_cuda,count,model_pcd,outer_model_ply,draw_reg)
            ...
            if view_video:
                ...
                viewer3d.update_cloud(undeformed_outer= t2_d_pcd_inner_cu.cpu(), deformed_outer = t2_d_pcd_outer_cu.cpu())
                # viewer3d.update_cloud(undeformed_outer= t2_d_pcd_inner_cu.cpu())
                viewer3d.tick()
