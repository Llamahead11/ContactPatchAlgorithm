...
def find_contact_points(def_pcd_last_layer,A,B,C,D,o3d_inv_full_T,centroid):
    ...
    if pcd_centre.point.positions.shape[0] != 0:
        ...
        for lug in num_lugs:
            ...
            if (lug_pcd.point.positions.shape[0] > 4) & (contact_patch.point.positions.shape[0] > 1050):
        ...
        cpa = np.sum([dA.sum() if np.any(dA) else 0 for dA in dA_list])
...
def calc_FEST_force(C01,C10):
    net_force_est_arr = []
    contact_patch_area_arr = []
    traction_arr = []
    data_files = [f"test_21/iteration_350.npz",f"test_9/iteration_270.npz", f"test_8/iteration_270.npz"]
    for i in range(0,3):
        ...
        undef_vol_first_layer = o3d.t.geometry.PointCloud(o3d.core.Device("CUDA:0"))
        undef_vol_first_layer.point.positions = o3d.core.Tensor.from_dlpack(volumetric_undef_points[1,:,:].reshape(-1,3).toDlpack())
        undef_vol_first_layer.point.normals = o3d.core.Tensor.from_dlpack(undef_vol_surface_normals[1,:,:].reshape(-1,3).toDlpack())
        ...
        #force_pre = force_preload(undef_vol_first_layer)
        ...
        if contact_flag == True:
            ...
            #force_est = np.zeros((1, 3))
            net_force_est = np.zeros(3)
...
#             continue
#         else:
#             net_force_est_loop = calc_FEST_force()
...
# def loss_fun(params):
#     C01,C10 = params
#     net_force_est_arr = calc_FEST_force(C01,C10)
#     net_force_est_arr = np.array(net_force_est_arr)
#     print(net_force_est_arr)
#     print(net_force_est_arr[0,2],net_force_est_arr[1,2])
#     err = (net_force_est_arr[0,2] - 7000)**2 + (net_force_est_arr[1,2] - 5000)**2
#     print("######################\n","The Params: ",params,"\n#####################")
#     print("######################\n","The Error: ",err,"\n#####################")
#     return err

# res = sciopt.minimize(loss_fun,x0 = [0.3e6,0.6e6])
# print(res)

target = np.array([-7000.0,-6000.0,-5000.0])  # target z-components for load index 0 and 1
sigma = np.array([100.0, 100.0, 100.0])     # measurement std for normalization

def loss_fun_log(x):
    # x are log-params
    logC01, logC10 = x
    C01, C10 = np.exp(logC01), np.exp(logC10)
    print("######################\n","The Params: ",C01,C10,"\n#####################")
    net_force_est_arr = calc_FEST_force(C01, C10)  # shape (2,3) expected
    net_force_est_arr = np.array(net_force_est_arr)
    #print(net_force_est_arr)
    sim_z = np.array([net_force_est_arr[0,2], net_force_est_arr[1,2], net_force_est_arr[2,2]])
    res = (sim_z - target) / sigma
    J = 0.5 * np.sum(res**2)

    # regulariser toward prior in log-space:
    prior = np.log(np.array([1e5, 1e5]))
    lam = 1e-2
    J += 0.5 * lam * np.sum((np.array([logC01, logC10]) - prior)**2)
    print("######################\n","The J: ",J,"\n#####################")
    return J

x0 = np.log(np.array([1e5, 1e5]))
#bounds = [(np.log(1e5), np.log(3e6)), (np.log(1e5), np.log(3e6))]
res = sciopt.minimize(loss_fun_log, x0=x0, method='Nelder-Mead',
               options={'maxiter': 2000, 'ftol': 1e-3})
C01_hat, C10_hat = np.exp(res.x)
print("fit done:", res.message)
print("C01, C10 =", C01_hat, C10_hat)
