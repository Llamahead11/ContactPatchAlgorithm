...
def register_t2cam_with_model(t2cam_pcd_cuda,model_pcd_cuda,corres_vector,p,q):
    """
...
def inner_deformed_to_inner_undeformed(prev_valid_mask,shared_stream,raycaster,inner_undef_lat,inner_undef,inner_lat,inner_arr,model_pcd_cuda,t2cam_pcd_cuda,count,model_pcd,model_ply,draw_reg):
    ...
    ICP_register_T2Cam_with_model(cropped_model_cuda,t2cam_pcd_cuda,d_t2_points,downsampled_cropped_model_cu,draw_reg)
    ...
    masked_pcd = t2_d_pcd_cu_t.select_by_mask(curr_valid_mask) #.voxel_down_sample(voxel_size = 0.002) 
    ...
    # centroid = masked_pcd.get_center()
    # pcd_centre = masked_pcd.clone()
    ...
    return curr_valid_mask, t2_und_t, np.max(d_dist), downsampled_cropped_model_cu, d_dist, t2_d_pcd_cu_t, masked_pcd#t2_d_pcd_cu_t #1#t2_d_pcd_def
...
def draw_lines_lineset(start_points, end_points, line_set):
    ...
    # valid_dist = dist < 0.6
    #mask = (line_start != 0.0).all(axis=1) & (line_end != 0.0).all(axis=1)
...
def main():
    ...
    t_1 = time.time()   
    tag_IDs, tag_locations = detector.process_3D_locations(vertex_map_gpu.as_tensor().cpu().numpy())
    t_2 = time.time() 
    ...
    register_t2cam_with_model(t2cam_pcd_cuda,model_pcd_cuda,correspondence_vector,p,q)
    time_at_registration = time.time()
    register_time = time_at_registration - time_at_corres_vec
    print("Registration in", register_time)

    prev_valid_mask, t2_d_pcd_inner_cu, max_inner_def, cropped_model_pcd_cu, d_inner_dist,t2_d_pcd_outer_cu, outer_select = inner_deformed_to_inner_undeformed(prev_valid_mask,shared_stream,raycaster,inner_undef_lat,inner_undef,inner_lat,inner_arr,model_pcd_cuda,t2cam_pcd_cuda,count,model_pcd,outer_model_ply,draw_reg)
    ...
    prev_outer_deformed = t2_d_pcd_outer_cu
    prev_outer_undeformed = t2_d_pcd_inner_cu
    prev_outer_select = outer_select
    ...
    try:
        while True:
            ...
            ## ROUGH ALIGNMENT WITH T2CAM USING APRILTAGS
            register_t2cam_with_model(curr_outer_pcd,model_pcd_cuda,correspondence_vector,p,q)
            time_at_registration = time.time()
            register_time = time_at_registration - time_at_corres_vec
            print("Registration in", register_time)

            curr_valid_mask,t2_d_pcd_inner_cu, max_inner_def, cropped_model_pcd_cu, d_inner_dist,t2_d_pcd_outer_cu, outer_select = inner_deformed_to_inner_undeformed(prev_valid_mask,shared_stream,raycaster,inner_undef_lat,inner_undef,inner_lat,inner_arr,model_pcd_cuda,curr_outer_pcd,count,model_pcd,outer_model_ply,draw_reg)
            time_at_Inner_c2c = time.time()
            c2c_dist_time = time_at_Inner_c2c - time_at_registration
            print("Inner and Outer C2C raycasting distance calculated in", c2c_dist_time)
            ...
            if view_video:
                draw_lines_lineset(prev_outer_deformed.select_by_mask(curr_valid_mask & prev_valid_mask).point.positions,t2_d_pcd_outer_cu.select_by_mask(curr_valid_mask & prev_valid_mask).point.positions,outer_disp)
